(() => {
  ...
  const tourSteps = [
    ...
    {
      kicker: "05 · VERIFIABLE OUTPUT",
      title: "결과를 <span>바로 확인할 수 있게 정리했습니다.</span>",
      copy: "논문 PDF, GitHub 구현, App Store 출시, 연구과제와 수상 이력을 한 포트폴리오 안에서 서로 연결했습니다. 각 프로젝트에서 관련 결과와 구현 자료로 바로 이동할 수 있습니다.",
      metrics: [["4.21/4.5", "GPA"], ["OPIc IH", "ENGLISH"], ["KIIT", "BEST PAPER GOLD"], ["GitHub/PDF", "LINKED OUTPUTS"]],
      proofs: [["publications", "Publications"], ["awards", "Awards"], ["contact", "Contact"]],
      highlight: "awards",
      speech: "마지막은 결과물이야. 논문, 구현, 출시 이력을 한곳에서 바로 확인할 수 있어.",
    },
  ];
  ...
  const careerNodes = [
    ...
    },
    {
      ...
      copy: "JOK/KSC/KCC/SAC selected publications, funded research, GitHub 구현 결과를 통해 Research-to-System 흐름을 결과 중심으로 정리하고 있습니다.",
      ...
      modal.innerHTML = `
        ...
        <section class="rs-dialog" role="dialog" aria-modal="true" aria-labelledby="rsDialogTitle">
          <header class="rs-dialog-head">
            <div class="rs-dialog-title"><i>✦</i><div><strong id="rsDialogTitle">Recruiter Suite</strong><small id="rsDialogSubtitle">RESEARCH-TO-SYSTEM PORTFOLIO</small></div></div>
  ...
  function renderTourStep(index, finished = false) {
    ...
    setDialog("30-sec Recruiter Mode", "AUTOMATIC RECRUITER TOUR");
  ...
  function renderMatcher() {
    setDialog("Job Match Analyzer", "LOCAL · PRIVATE · PORTFOLIO SIGNALS ONLY");
    content.innerHTML = `
    ...
    speak("채용공고를 붙여 넣으면, 내 포트폴리오에서 직접 확인되는 기술만 골라서 비교해줄게.");
  }
  ...
  function analyzeJD() {
    ...
    const gapChips = gaps.length ? gaps.map((x) => `<span class="rs-chip gap">${x.label}</span>`).join("") : `<span class="rs-chip">No detected gaps</span>`;
    const proofRoutes = [...new Map(matched.map((x) => [x.route, x])).values()].slice(0, 5);
    const proofButtons = proofRoutes.map((x) => `<button type="button" data-rs-proof="${x.route}">${x.label} ↗</button>`).join("");

    results.innerHTML = `
      <div class="rs-score-head">
        <div class="rs-score-orb" style="--score:${score}%"><strong>${score}</strong></div>
        <div class="rs-score-label"><strong>Portfolio fit signal</strong><span>인식된 JD 기술 키워드 중 현재 공개 포트폴리오에서 직접 확인되는 비율입니다.</span></div>
      </div>
      <div class="rs-category-list">${categoryRows || `<div class="rs-match-empty">카테고리별 신호가 충분하지 않습니다.</div>`}</div>
      <div class="rs-match-section"><span>MATCHED PORTFOLIO SIGNALS</span><div class="rs-chip-list">${matchedChips}</div></div>
      <div class="rs-match-section"><span>NOT DIRECTLY SHOWN IN CURRENT PORTFOLIO</span><div class="rs-chip-list">${gapChips}</div></div>
      <div class="rs-proof-row">${proofButtons}</div>`;
    speak(`이 JD에서는 공개 포트폴리오 기준 ${matched.length}개의 기술이 직접 연결돼.`, 3600);
  }
  ...
  function renderCareer() {
    setDialog("Interactive Career Graph", "PRODUCT → RESEARCH → EDGE → OUTPUT");
  ...
  function renderCareerDetail(index) {
    ...
    detail.innerHTML = `
      ...
      <div class="rs-proof-row"><button type="button" data-rs-proof="${node.command}">Open related work ↗</button></div>`;
  }
